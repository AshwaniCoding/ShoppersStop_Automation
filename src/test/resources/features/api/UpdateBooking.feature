...
Feature: Validate Update Booking Endpoint
  ...
  Scenario: Verify user can successfully update an existing booking
  ...
  Scenario: Verify user can partially update an existing booking
    Given user wants to call "/booking/{id}" end point
    And set header "Content-Type" to "application/json"
    And set header "Accept" to "application/json"
    And set header "Cookie" to "token=<auth.token>"
    And set path parameter "id" to "stored.booking.id"
    And set request body from file "partial_update_booking.json"
    When user performs patch call
    Then verify status code is 200
    And verify the "firstname" in the response body is "James"